...
(ma_fit_2 = Arima(windspeed_monthly_s, order=c(0,0,12)))
...
plot(forecast(ma_fit_2, 50), sub = root)
dev.off()
...
plot(forecast(ma_fit_2,30))

plot(monthly_data$windspeed,type="l",col="red")

fc=forecast(ma_fit_2,30)
...
accuracy(fc$mean, windspeed_daily_s[151:180])
...
(ar_fit_2 <- Arima(windspeed_monthly_s[1:150], order=c(12,0,0)))
(ar_fit_3 <- Arima(windspeed_monthly_s[1:150], order=c(13,0,0)))
```

```{r}
png(filename = paste(root, "Forecast_monthly_MA", ".png", sep = ""), width = 720, height = 500)
plot(forecast(ar_fit_2, 50), sub = root)
dev.off()
...
```{r}
ind <- seq(as.Date("2000-01-01"), as.Date("2014-12-31"), by = "month")
plot(forecast(ar_fit_2,30))

plot(monthly_data$windspeed,type="l",col="red")

fc=forecast(ar_fit_2,30)
ind <- seq(as.Date("2012-07-01"), as.Date("2014-12-31"), by = "month")
plot(x=ind,y=fc$mean,type="l",col="blue")
lines(x=ind,y=monthly_data$windspeed[151:180],col="red")
legend("topleft",c("predicted","Actual"),fill=c("blue","red"))
...
plot(forecast(ar_fit_3), sub = root)
dev.off()
```
